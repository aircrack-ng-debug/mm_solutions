...
// --- Burger Icon Component (bleibt unverändert) ---
...
// --- Mobile Menu Overlay Component (Logik für /home bleibt, Anpassung des Breakpoints für Sichtbarkeit) ---
const MobileMenu = ({ isOpen, onClose, navLinks, handleContactScroll, location, activeNavHash, setActiveNavHash, navigate }) => {
        ...
        if (link.isContact) {
            ...
            if (location.pathname === '/home') {
                ...
                if (section) {
                    ...
                    // --- ÄNDERUNG HIER ---
                    // Die Logik für den Header-Offset im Mobile-Menü muss den neuen Breakpoint für die Pill-Nav berücksichtigen,
                    // falls die Header-Höhe oder Pill-Nav-Höhe unterschiedlich für Tablets vs. Mobile ist.
                    // Fürs Erste belassen wir es, da der Header für Mobile relevant ist.
    ...
    return (
        // --- ÄNDERUNG HIER: Mobile Menu wird bis lg angezeigt (lg:hidden) ---
        <div className="fixed inset-0 bg-white z-40 flex flex-col items-center justify-center p-6 lg:hidden">
            <nav className="flex flex-col space-y-4 w-full max-w-xs text-center">
                {navLinks.map(link => (
                    <a
                        key={link.id}
                        href={link.isRouterLink ? link.href : (location.pathname === '/home' && link.href.startsWith('#') ? link.href : `/home${link.href}`)}
                        ...
                        className={`${linkBaseClasses} ${
                            link.isContact ? 'bg-blue-600 text-white hover:bg-blue-700 hover:text-white' :
                                ...
                                (!link.isRouterLink && location.pathname === '/home' && activeNavHash === link.href)
                                    ? activeLinkClasses : ''
                        }`}
                    >
                        {link.label}
...
const Layout = ({ children }) => {
    ...
    useEffect(() => {
        const calculateLayoutParameters = () => {
            ...
            // --- ÄNDERUNG HIER: isLargeScreen für Pill-Nav Logik ---
            // Statt isMobileView (<768px), verwenden wir eine Logik, die berücksichtigt, wann die Pill-Nav wirklich aktiv ist (ab lg / 1024px)
            const isPillNavActiveScreen = window.innerWidth >= 1024; // lg Breakpoint
            let navObstructionHeightValue = 0;

            if (!isPillNavActiveScreen) { // Gilt für Mobile UND Tablet (bis lg)
                navObstructionHeightValue = currentHeaderHeight;
                setPillStickyOffset(8); // Standard-Offset, falls Pill-Nav nicht da oder anders behandelt
                setPillMarginTop(0);    // Kein spezieller Margin für Pill-Nav
            } else { // Gilt für Laptop/Desktop (ab lg)
    ...
    }, [isMounted, location.pathname, headerHeight, pillNavHeight]); // Dependencies angepasst
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            // --- ÄNDERUNG HIER: isLargeScreen für Offset-Berechnung ---
            const isPillNavActiveScreen = window.innerWidth >= 1024; // lg Breakpoint

            if (!isPillNavActiveScreen) { // Mobile und Tablet bis lg
                scrollTriggerOffset = headerHeight + 20; // Nur Header-Höhe relevant
            } else { // Laptop/Desktop ab lg
                scrollTriggerOffset = pillStickyOffset + pillNavHeight + 40; // Pill-Nav Höhe relevant
            }
    ...
    const scrollToSectionHandler = (sectionId) => {
        ...
        if (section) {
            ...
            if (!isPillNavActiveScreen) { // Mobile und Tablet
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-white text-gray-800 flex flex-col">
                ...
                >
                    <RouterLink to="/home" className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                    ...
                    {/* --- ÄNDERUNG HIER: Burger Icon wird bis lg angezeigt (lg:hidden) --- */}
                    <div className="lg:hidden">
                        <BurgerIcon onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} isOpen={isMobileMenuOpen} />
                    </div>
                </header>

                {/* --- ÄNDERUNG HIER: Pill Nav wird ab lg angezeigt (hidden lg:flex) --- */}
                <nav
                    ref={pillNavRef}
                    className={`hidden lg:flex w-full justify-center py-2 fade-in-up ${isMounted ? 'fade-in-up-mounted' : ''} z-40`}
                    style={{
                        // Sticky-Logik muss den neuen Breakpoint für die Pill-Nav berücksichtigen
                        // Die Berechnung von pillStickyOffset und pillMarginTop in useEffect wurde angepasst
                        top: `${pillStickyOffset}px`,
                        marginTop: `${pillMarginTop}px`,
                        animationDelay: '0.1s',
                        position: 'sticky', // Sicherstellen, dass es sticky ist, wenn es angezeigt wird
                    }}
                >
...
// --- Main App Component (bleibt unverändert) ---
                ...
                <Routes>
                    ...
                    <Route path="*" element={
                        <div className="min-h-screen h-screen flex flex-col items-center justify-center text-center py-10 px-4">
                            ...
                            <RouterLink to="/home" className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                                Go to Homepage
                            </RouterLink>
                        </div>
                    } />
